...
declare
  ...
  -- On Monday before 9 AM, check PREVIOUS week (for "close last week" banner)
  week_to_check date := CASE 
    WHEN EXTRACT(DOW FROM now()) = 1 AND EXTRACT(HOUR FROM now()) < 9 
    THEN week_start - interval '7 days'
    ELSE week_start
  END;
  result jsonb;
begin
  ...
  -- Check if the relevant week is closed (previous week on Monday AM, current week otherwise)
  week_closed as (
    select exists(
