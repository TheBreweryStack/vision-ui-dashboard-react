...
begin
  ...
  -- Get user's timezone from account_settings (default to America/New_York)
  SELECT COALESCE(timezone, 'America/New_York') INTO user_tz
  FROM account_settings
  WHERE user_id = uid
  LIMIT 1;
  
  -- If no account_settings row exists, use default
  ...
  -- On Monday before 9 AM in USER's timezone, check PREVIOUS week
